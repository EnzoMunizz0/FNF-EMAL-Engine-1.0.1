...
HXLINE(  37)				message = HX_("Info ",d2,97,43,51);
            			}
HXLINE(  39)			if ((type == gl->VERTEX_SHADER)) {
HXLINE(  39)				message = HX_("compiling vertex shader",b1,d1,76,80);
            			}
            			else {
HXLINE(  40)				if ((type == gl->FRAGMENT_SHADER)) {
HXLINE(  40)					message = HX_("compiling fragment shader",c5,e6,20,4f);
            				}
            				else {
HXLINE(  42)					message = HX_("compiling unknown shader type",4f,b9,2c,ae);
            				}
            			}
HXLINE(  44)			message = (message + (HX_("\n",0a,00,00,00) + shaderInfoLog));
HXLINE(  46)			if ((compileStatus == 0)) {
...
HXLINE(  49)					::lime::utils::Log_obj::debug(message,::hx::SourceInfo(HX_("lime/graphics/opengl/GLShader.hx",be,a5,56,4f),49,HX_("lime.graphics.opengl._GLShader.GLShader_Impl_",44,46,91,94),HX_("fromSource",e5,b5,75,75)));
            				}
            			}
            		}
HXLINE(  52)		return shader;
            	}
