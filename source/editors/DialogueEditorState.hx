...
class DialogueEditorState extends MusicBeatState
{
	...
	function updateTextBox() {
		...
		DialogueBoxEmal.updateBoxOffsets(box);
	}
	...
	function reloadCharacter() {
		...
		character.x = DialogueBoxEmal.LEFT_CHAR_X;
		character.y = DialogueBoxEmal.DEFAULT_CHAR_Y;

		switch(character.jsonFile.dialogue_pos) {
			case 'right':
				character.x = FlxG.width - character.width + DialogueBoxEmal.RIGHT_CHAR_X;
	...
	function reloadText(speed:Float = 0.05) {
		...
		daText = new Alphabet(DialogueBoxEmal.DEFAULT_TEXT_X, DialogueBoxEmal.DEFAULT_TEXT_Y, textToType, false, true, speed, 0.7);
		add(daText);
